...
export const performanceService = {
  ...
  async getPerformanceStats(userId: string) {
    ...
    const wpmValues = data.map((session: any) => session.wpm)
    const accuracyValues = data.map((session: any) => session.akurasi)

    return {
      bestWpm: Math.max(...wpmValues),
      averageWpm: Math.round(wpmValues.reduce((sum: number, wpm: number) => sum + wpm, 0) / wpmValues.length),
      bestAccuracy: Math.max(...accuracyValues),
      averageAccuracy: Math.round(accuracyValues.reduce((sum: number, acc: number) => sum + acc, 0) / accuracyValues.length),
